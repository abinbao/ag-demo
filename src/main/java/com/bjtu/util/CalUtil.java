...
public class CalUtil {
	 ...
	 * @return
	 */
	public static List<Square> divideGridFir(int m1, double xLength,
			double yLength) {
		...
		for (int i = 0; i < m1; i++) {
			double temp = yBottom;
			for (int j = 0; j < m1; j++) {
				Square square = new Square(Config.formatDouble(xBottom), Double.parseDouble(nbf
						.format(xBottom + xUnit)), Config.formatDouble(temp), Double.parseDouble(nbf
						.format(temp + yUnit)));
	...
	public static void divideGridSec(Square square) {
		...
		for (int i = 0; i < m2; i++) {
			double temp = yBottom;
			for (int j = 0; j < m2; j++) {
				Square squareTemp = new Square(Config.formatDouble(xBottom), Double.parseDouble(nbf
						.format(xBottom + xUnit)), Config.formatDouble(temp), Double.parseDouble(nbf
						.format(temp + yUnit)));
				temp = Double.parseDouble(nbf.format(temp + yUnit));
	 ...
	 *            点集合
	 */
	public static void calPointNum(List<Square> squareList,
			List<Point> pointList) {
		ArrayList<Point> temp = (ArrayList<Point>) pointList;

		for (Square square : squareList) {
			for (Point point : temp) {
				if (point.getX() >= square.getX1()
						&& point.getX() <= square.getX2()
						&& point.getY() >= square.getY1()
	...
	public static int searchActualPointNum(Square query, List<Point> pointList) {
		...
		for (Point point : pointList) {
			...
			if (x >= query.getX1() && x <= query.getX2() && y >= query.getY1()
					&& y <= query.getY2()) {
				count = count + 1;
			}
	 ...
	 *            查询区域
	 */
	public static void searchLapalcePointNum(Square square,
			Map<String, Double> Agcount, Square query) {
		...
		if (!square.isFlag()) {
			...
			// 2. 左上角
			else if (x3 <= x1 && x4 >= x1 && x4 <= x2 && y3 >= y1 && y3 <= y2
					&& y4 >= y2) {
			...
			// 4. 左下角
			else if (x3 <= x1 && x4 >= x1 && x4 <= x2 && y3 <= y1 && y4 >= y1
					&& y4 <= y2) {
			...
			// 6.右下角
			else if (x3 >= x1 && x3 <= x2 && x4 >= x2 && y3 <= y1 && y4 >= y1
					&& y4 <= y2) {
			...
			// 8. 右上角
			else if (x3 >= x1 && x3 <= x2 && x4 >= x2 && y3 >= y1 && y3 <= y2
					&& y4 >= y2) {
			...
			logger.info(square.toString() + "===" + Agcount.get("agNum")
					+ "==rate==" + rate + "== count_lap=="
			...
			// }
			// }
			searchLapalcePointNum(square.getSquare_D1(), Agcount,
					Config.querySquare);
			searchLapalcePointNum(square.getSquare_D2(), Agcount,
					Config.querySquare);
		}
	}
	...
	public static void judgeDivideOld(double e, Square square) {
		...
		if (!square.getSquareList().isEmpty()) {
			for (Square item : square.getSquareList()) {
				temp = temp
						+ Math.abs((double) item.getCount() - square.getAvg());
			}
	...
	public static void findGoldLine(Square square, List<Point> pointList) {
		...
		if (!square.getLineXList().isEmpty() && square.getLineYList().isEmpty()) {
			...
			for (int i = 1; i < square.getLineXList().size(); i++) {
				int temp = calLineXValue(square.getLineXList().get(i), square,
						pointList);
				if (temp < xValue) {
			...
			Square squareD2two = new Square(goldLineX.getX1(), square.getX2(),
					Config.formatDouble(square.getY1()), Config.formatDouble(square.getY2()));
		...
		if (square.getLineXList().isEmpty() && !square.getLineYList().isEmpty()) {
			...
			for (int i = 1; i < square.getLineYList().size(); i++) {
				int temp = calLineYValue(square.getLineYList().get(i), square,
						pointList);
				if (temp < yValue) {
		...
		if (!square.getLineXList().isEmpty()
				&& !square.getLineYList().isEmpty()) {
			...
			if (xValue <= yValue) {
				...
				Square squareD1one = new Square(Config.formatDouble(square.getX1()),
						Config.formatDouble(goldLineX.getX1()), Config.formatDouble(square.getY1()), Config.formatDouble(square.getY2()));
				Square squareD2two = new Square(goldLineX.getX1(),
						square.getX2(), square.getY1(), square.getY2());
				square.setSquare_D1(squareD1one);
				square.setSquare_D2(squareD2two);
			} else {
				square.setGold_y_line(goldLineY);
			...
			setLineToSubSuqare(square);
			logger.info(square.toString() + "划分合并的区域："
					+ square.getSquare_D1().toString()
	...
	public static void setLineToSubSuqare(Square square) {
		...
		for (Square item : square.getSquareList()) {
			if (item.getX1() >= square.getSquare_D1().getX1()
					&& item.getX2() <= square.getSquare_D1().getX2()
					&& item.getY1() >= square.getSquare_D1().getY1()
					&& item.getY2() <= square.getSquare_D1().getY2()) {
				square.getSquare_D1().getSquareList().add(item);
				square.getSquare_D1().setCount(
						square.getSquare_D1().getCount() + item.getCount());
				square.getSquare_D1().setCountLa(
						square.getSquare_D1().getCountLa() + item.getCountLa());
			} else {
				...
				square.getSquare_D2().setCount(
						square.getSquare_D2().getCount() + item.getCount());
				square.getSquare_D2().setCountLa(
						square.getSquare_D2().getCountLa() + item.getCountLa());
			}
		}

		// 将上次分割的线添加到 square_D(x) 中
		for (LineX lineX : square.getLineXList()) {
			if (lineX.getX1() > square.getSquare_D1().getX1()
					&& lineX.getX1() < square.getSquare_D1().getX2()) {
				square.getSquare_D1().getLineXList().add(lineX);
			}
			if (lineX.getX1() > square.getSquare_D2().getX1()
					&& lineX.getX1() < square.getSquare_D2().getX2()) {
				square.getSquare_D2().getLineXList().add(lineX);
			}
			
		}
		for (LineY lineY : square.getLineYList()) {
			if (lineY.getY1() > square.getSquare_D1().getY1()
					&& lineY.getY1() < square.getSquare_D1().getY2()) {
				square.getSquare_D1().getLineYList().add(lineY);
			}
			if (lineY.getY1() > square.getSquare_D2().getY1()
					&& lineY.getY1() < square.getSquare_D2().getY2()) {
		...
		if (!square.getSquare_D1().getSquareList().isEmpty()) {
			avgD1 = (double) (square.getSquare_D1().getCount())
					/ (square.getSquare_D1().getSquareList().size());
		} else {
			avgD1 = 0;
		}
		double avgD2;
		if (!square.getSquare_D2().getSquareList().isEmpty()) {
			avgD2 = (double) (square.getSquare_D2().getCount())
					/ (square.getSquare_D2().getSquareList().size());
		} else {
			avgD2 = 0;
		}
	 ...
	 * @param pointList
	 */
	public static int calLineXValue(LineX lineX, Square square,
			List<Point> pointList) {
		...
		for (Point point : pointList) {
			if (point.getX() >= square.getX1() && point.getX() <= lineX.getX1()
					&& point.getY() >= square.getY1()
					&& point.getY() <= square.getY2()) {
				pointD1 = pointD1 + 1;
			}
		...
		logger.info(square.toString() + "===" + lineX.toString()
				+ "==pointD2==" + pointD2);
		if (!square.getSquareList().isEmpty()) {
			for (Square item : square.getSquareList()) {
				if (item.getX2() <= lineX.getX1()
						&& item.getX1() >= square.getX1()
						&& item.getY1() >= square.getY1()
	 ...
	 * @param pointList
	 */
	public static int calLineYValue(LineY lineY, Square square,
			List<Point> pointList) {
		int pointD1 = 0; // D1 区域点的个数 v1
		int pointD2 = 0; // D2 区域点的个数 v2
		...
		for (Point point : pointList) {
			if (point.getX() >= square.getX1()
					&& point.getX() <= square.getX2()
					&& point.getY() >= lineY.getY1()
					&& point.getY() <= square.getY2()) {
				pointD1 = pointD1 + 1;
			}
		}
		pointD2 = square.getCount() - pointD1;
		logger.info(square.toString() + "===" + lineY.toString()
				+ "==pointD2==" + pointD2);
		if (!square.getSquareList().isEmpty()) {
			for (Square item : square.getSquareList()) {
				if (item.getX2() <= square.getX2()
						&& item.getX1() <= square.getX1()
						&& item.getY1() >= lineY.getY1()
						&& item.getY2() <= square.getY2()) {
					squareD1 = squareD1 + 1;
				} else {
					squareD2 = squareD2 + 1;
