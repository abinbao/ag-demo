...
public class Canopy {
    ...
    private ArrayList<float[]> points = (ArrayList) new ArrayList<>(); // 进行聚类的点
    private ArrayList<ArrayList<float[]>> clusters = (ArrayList) new ArrayList<>(); // 存储簇

    private double T2 = -1; // 阈值
    int clnm;

    /* 构造函数 */
    public Canopy(ArrayList dataSet) {
    ...
    public int cluster() {
        ...
        while (points.size() != 0) {/* point不为空 */
            ...
            while (index < points.size()) {
                 ...
                 * (basePoint.x – anotherPoint.x) + (basePoint.y –
                 * anotherPoint.y) (basePoint.y – anotherPoint.y));
                 */
                float sum = 0;
    ...
    private double getAverageDistance(ArrayList<float[]> points) {
        ...
        for (int i = 0; i < pointSize; i++) {
            for (int j = 0; j < pointSize; j++) {
                ...
                /*
                 * sum += Math.sqrt((pointA.x – pointB.x) * (pointA.x –
                 * pointB.x) + (pointA.y – pointB.y) * (pointA.y – pointB.y));
                 */
            }
        }
    ...
    private float[] getCenterPoint(ArrayList<float[]> points) {
        ...
        /*
         * double sumX = 0; double sumY = 0; for (flout[] point : points) { sumX
         * += point.x; sumY += point.y; } int clusterSize = points.size(); Point
         * centerPoint = new Point(sumX / clusterSize, sumY / clusterSize);
         */
